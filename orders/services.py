...
def create_order(user, table_id: int, items: list):
    with transaction.atomic():
        ...
        order = Order.objects.create(
            total_sum=Decimal("0.00"), table=table_instance, user=user
        )
        ...
        for item in items:
            ...
            order_item = OrderItem.objects.create(
                order=order, dish=dish, quantity=quantity, saved_price=dish.price
            )
...
def accrue_bonus(order):
    ...
    client_bonus = ClientBonus.objects.create(
        client=user, order=order, bonuses=calculate_bonuses
    )
